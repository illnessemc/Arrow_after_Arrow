...
> Python + Pygame 开发的箭头路径解谜游戏。当前版本：`v1.0`
...
棋盘中分布着朝向上、下、左、右的箭头。基础模式使用单格直箭头，进阶模式
加入多格直线和折线箭头。点击箭头后，程序会检查箭头头部到棋盘边界之间的
路径：前方没有其他箭头时，箭头飞出；存在阻挡时，第一支阻挡箭头会变红并
震动，同时扣除一次失误机会。
...
- 开始、选关、游戏、胜利和失败界面；
- 基础、进阶和无尽三种模式，固定模式拥有独立选关目录；
- 鼠标点击箭头任意线段即可选择整支箭头；
- 支持单格、多格直线和多转角折线箭头；
- 正确处理四个方向的路径阻挡与边界出界；
- 箭头飞出、圆角滑动、棋盘点拖尾和碰撞震动反馈；
- 三次失误机会、倒计时、重新开始和关卡切换；
- 3 个手工单格基础关和 6 个进阶关，全部经过可通关性验证。

### 扩展功能

- **主题关卡**：潮汐回廊、四向机关、同心星环、虹桥织网和星门终阵；
- **分级玩法**：基础模式练习单格路径判断，进阶模式挑战多格折线依赖；
...
- 基础关时限依次为 45、70 和 90 秒；
- 进阶关时限依次为 60、110、125、155、165 和 180 秒；
...
测试覆盖三种模式入口、单格基础关、正常消除、阻挡扣除、边界出界、通关切换、
失败重开、关卡完整覆盖、可通关性、倒计时、星级、无尽累计分和最高分存储。
全部通过时终端最后显示：
